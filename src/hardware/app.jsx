      ...
      useEffect(() => {
        const offs = [
          ...
          kernelBus.on("camera:photo-captured", (p) =>
            syslogsApi.info("com.rainos.camera", "photo", `Foto capturada (${p.width}x${p.height})`, p)
          ),
      ...
      useEffect(() => {
        const interval = setInterval(async () => {
          try {
            await networkApi.ping("https://cloudflare.com");
          } catch {}
        }, 60000);
        return () => clearInterval(interval);
      }, [networkApi]);
      ...
      useEffect(() => {
        const interval = setInterval(() => {
          // Cada 45s, 20% de probabilidad de conectar/desconectar algo
          if (Math.random() > 0.8) {
            const devices = usbApi.listDevices();
            if (devices.length > 0 && Math.random() > 0.5) {
              const random = devices[Math.floor(Math.random() * devices.length)];
              usbApi.detachDevice(random.id);
            }
          }
        }, 45000);
        return () => clearInterval(interval);
            ...
            <Spotlight
              ...
              onRunAction={async (action) => {
                ...
                if (action.id === "speedtest") {
                  ...
                  syslogsApi.info("com.rainos.network", "speedtest",
                    `Resultado: ${result?.downloadMbps?.toFixed(2) ?? "?"} Mbps`, result);
                }
                syslogsApi.info(
...
function SystemBridge({ children }) {
  ...
  useEffect(() => {
    const offs = [
      ...
      kernelBus.on("thermal:critical-reached", (p) =>
        syslogsApi.error("com.rainos.thermal", "critical", `Nivel crítico alcanzado (${
    }`
